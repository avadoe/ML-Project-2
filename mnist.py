...
for train_idx, test_idx in kf.split(X_train):
    ...
    y_cv_train, y_cv_test = y_train[train_idx], y_test[test_idx]
    for i, architecture in enumerate(architectures):
        ...
        for activation in activation_functions:
            for output_fn in output_functions:
                ...
                accuracy = accuracy_score(np.argmax(y_test, axis=1), y_pred=y_predict)
                cm = confusion_matrix(np.argmax(y_test, axis=1), y_pred=y_predict)
